"""
Backtester Magic Formula US (S&P 500).
Mesma logica do BR: ranking mensal EV/EBIT + ROIC, hold buffer, rebalance mensal.
Benchmark: S&P 500 (^GSPC). Risk-free: T-Bill 13w (^IRX).

Limitacao: survivorship bias — universo fixo nos tickers atuais do S&P 500.
"""
...
def _fetch_fundamentals(ticker: str) -> dict:
    """
    Fundamentals historicos para backtest:
    - ebit_q: EBIT trimestral
    - ebit_a: EBIT anual (fallback)
...
def _ttm_ebit_at(fund: dict, cutoff: date) -> float | None:
    """TTM EBIT em cutoff. Prioridade: 4 trimestres. Fallback: anual."""
...
def fetch_prices(tickers: list[str], start: str, end: str) -> pd.DataFrame:
    """Retorna DataFrame de precos mensais. Inclui SPX (^GSPC) e TBILL (^IRX)."""
    print(f"[backtest_us] Baixando precos mensais para {len(tickers)} tickers + ^GSPC + ^IRX...")
    all_tickers = tickers + ["^GSPC", "^IRX"]
...
def run_backtest(
    ...
    if rebal_dates.empty:
        print("[backtest_us] Nenhuma data de rebalanceamento no periodo")
        return {}
    ...
    for i, rebal_dt in enumerate(rebal_dates[:-1]):
        ...
        # S&P 500 retorno mensal
        spx_ret = None
        if "SPX" in prices_df.columns:
            s0 = _safe_float(prices_df.loc[prices_df.index <= rebal_dt, "SPX"].iloc[-1]) if not prices_df.loc[prices_df.index <= rebal_dt, "SPX"].empty else None
            s1 = _safe_float(prices_df.loc[prices_df.index <= next_dt, "SPX"].iloc[-1]) if not prices_df.loc[prices_df.index <= next_dt, "SPX"].empty else None
            if s0 and s1 and s0 > 0:
                spx_ret = (s1 - s0) / s0 * 100
                if spx_base is None:
                    spx_base = s0

        # T-Bill retorno mensal (^IRX = annualized yield %)
        tbill_ret = None
        if "TBILL" in prices_df.columns:
            tbill_ann = _safe_float(prices_df.loc[prices_df.index <= rebal_dt, "TBILL"].iloc[-1]) if not prices_df.loc[prices_df.index <= rebal_dt, "TBILL"].empty else None
        ...
        monthly_results.append({
            ...
            "spx_retorno_mes_pct": round(spx_ret, 2) if spx_ret is not None else None,
    ...
    for r in monthly_results:
        if r["spx_retorno_mes_pct"] is not None:
            spx_acc *= (1 + r["spx_retorno_mes_pct"] / 100)
        spx_vals.append(round(spx_acc, 2))
    spx_total = spx_acc - 100
    ...
    stats = {
        ...
        "spx_total_pct": round(spx_total, 2),
