...
def _build_card_us(c, ag, rank, group):
    ticker = c["TICKER"]
    ...
    web_html = (
        ...
        if web_res and "sem dados" not in web_res.lower() and "no web" not in web_res.lower() else ""
    )
    ...
    <div class="col">
      <div class="card h-100" style="background:#161b22;border:1px solid #30363d;border-top:3px solid {border_color}">
        <div class="card-header d-flex align-items-center gap-2 py-2" style="background:#0d1117;border-bottom:1px solid #30363d">
          ...
          <span class="fw-bold" style="font-size:1.1rem;color:#58a6ff">{ticker}</span>
...
def _table_row_us(c, ag, rank, group):
    ticker = c["TICKER"]
    pos = c.get("posicao_mf", "—")
    ev = c.get("EV/EBIT")
    roic = c.get("ROIC")
    ...
    <tr>
      ...
      <td>
        <a href="https://finance.yahoo.com/quote/{ticker}" target="_blank" rel="noopener"
           class="fw-bold text-decoration-none" style="color:#58a6ff">{ticker}</a>
      </td>
...
def generate_html_us(data: dict, analyses: dict, output_path: str, top_n: int = 20):
    ...
    rows_html = (
        f'<tr style="background:#0a2218"><td colspan="16" class="py-2 px-3 fw-bold" style="color:#3fb950;font-size:.75rem;letter-spacing:.5px">▼ COMPRAR — {n_comprar} ações</td></tr>'
        + "".join(_table_row_us(cand_lookup[t], ag, r+1, "COMPRAR") for r,(t,ag,*_) in enumerate(comprar_list))
        ...
        + "".join(_table_row_us(cand_lookup[t], ag, r+1, "NEUTRO") for r,(t,ag,*_) in enumerate(neutro_list))
    )

    comprar_cards = "".join(_build_card_us(cand_lookup[t], ag, r+1, "COMPRAR") for r,(t,ag,*_) in enumerate(comprar_list))
    neutro_cards  = "".join(_build_card_us(cand_lookup[t], ag, r+1, "NEUTRO")  for r,(t,ag,*_) in enumerate(neutro_list))
...
<div class="hero">
  <div class="container-fluid px-3 px-md-4">
    <h1>Magic Formula <em>US</em> &#127482;&#127480; — S&amp;P 500</h1>
    <p class="text-light opacity-75 mb-3" style="font-size:.85rem">
      {now} · yfinance · S&amp;P 500 (~500 ações) · Metodologia Greenblatt · Análise Claude AI
    </p>
    <div class="d-flex flex-wrap gap-2">
      <div class="stat-pill"><div class="val">{total}</div><div class="lbl">S&amp;P 500 total</div></div>
...
<footer>Magic Formula US · {now} · Não é recomendação de investimento.</footer>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer py-2">
        <a id="modalYFBtn" href="#" target="_blank" rel="noopener"
           class="btn btn-primary w-100 fw-bold">↗ Abrir no Yahoo Finance</a>
      </div>
    </div>
  </div>
</div>
...
new Chart(document.getElementById('chartScatter'), {{
  ...
  options: {{
    ...
    scales: {{
      x: {{ title: {{ display: true, text: 'EV/EBIT', color: '#6e7681' }}, ticks: {{ color: '#8b949e' }}, grid: {{ color: '#30363d' }} }},
      y: {{ title: {{ display: true, text: 'ROIC %', color: '#6e7681' }}, ticks: {{ color: '#8b949e', callback: v => v + '%' }}, grid: {{ color: '#30363d' }} }}
    }}
  }}
}});
...
function openModal(ticker) {{
  ...
  $('#modalScore').html(`Score IA: <strong style="color:#bc8cff">${{d.score_ia ?? '—'}}/10</strong>`);
  ...
  $('#modalMetrics').html(metrics.map(([l,v,c]) =>
    `<div class="col-6 col-sm-4 col-md-3"><div class="modal-metric">
     <label>${{l}}</label><span style="color:${{c||'#e6edf3'}}">${{v}}</span></div></div>`
  ).join(''));
  ...
  bootstrap.Modal.getOrCreateInstance(document.getElementById('tickerModal')).show();
    ...
    print(f"[report_us] HTML gerado: {output_path}")
...
if __name__ == "__main__":
    ...
    out = args.output or str(Path(args.json).parent / "relatorio_us.html")
    generate_html_us(data, analyses, out, top_n=args.top)
